...
interface NewPostProps {
    ...
    allLabels: label[],
    selectedLabelsNp: label[],
    setSelectedLabelsNp: React.Dispatch<React.SetStateAction<label[]>>,
    newLabel: string,
    setNewLabel: React.Dispatch<React.SetStateAction<string>>
};

const NewPost = ({ handleSubmit, title, setTitle, body, setBody, handleSubmitForm, allLabels, selectedLabelsNp, setSelectedLabelsNp, newLabel, setNewLabel }: NewPostProps) => {
    ...
    return (
        <div className="w-full px-8">
            ...
            <form onSubmit={handleSubmit}>
                ...
                { selectedLabelsNp.length
                    ?
                    <div className="flex flex-row flex-wrap border-4 border-emerald-500 my-2">
                        {selectedLabelsNp.map(label => <div onClick={(e) => setSelectedLabelsNp(selectedLabelsNp.filter(labele => labele.id !== label.id))} className="bg-orange-200 mx-3 my-1 px-2 rounded-full cursor-pointer hover:bg-orange-300">{label.name}</div>)}
                    </div>
                    :
                    <></>
                } 
                ...
                { allLabels.filter(labele => !(selectedLabelsNp.some(l => l.id === labele.id))).length
                    ?
                    <div className="flex flex-row flex-wrap border-4 border-indigo-200">
                        {allLabels.filter(labele => !(selectedLabelsNp.some(l => l.id === labele.id)))
                        .map(label => <div onClick={(e) => setSelectedLabelsNp([...selectedLabelsNp, label])} className="bg-orange-200 mx-3 my-1 px-2 rounded-full cursor-pointer hover:bg-orange-300">{label.name}</div>)}
                    </div>
                    :
                    <></>
                }
